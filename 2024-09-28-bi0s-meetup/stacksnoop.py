...
# dictionary with PIDs and stack traces
event_pid_dict = {}
...
def print_event(cpu, data, size):
    ...
    try:
        assert(event_pid_dict[event.pid] == event.stack_id)
    except AssertionError:
        print(f"Rootkit activity detected, stack_id: {event.stack_id} does not match stored ID: {event_pid_dict[event.pid]}")
    except KeyError:
        event_pid_dict[event.pid] = event.stack_id
    if verbose:
